...
def create_recipe():
    if request.method == 'POST':
        ...
        MyIngredient = request.form.get('Ingredient')

        #check photo, user cant upload recipe without upload photo
        if request.method == 'POST':
            if 'file' not in request.files:
                flash('No file part')
                return redirect(request.url)
        ...
        label = 0
        if button3 != None:
            create_recipe(RecipeName,Description,Serving,image_datas)
            recipe_id = Recipes.query.filter_by(name= RecipeName).last().id
            image = base64.b64encode(image_datas).decode('ascii')
            return render_template("recipe.html", user=current_user, image=image, Descriptions=Description, 
            RecipeName = RecipeName, MyIngredient = MyIngredient, Step_number = Step_number)
        else:
